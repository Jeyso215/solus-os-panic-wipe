...
void write_erase_notice(const char *v, mode_t mode){
    write_message_to_block_device("Panic handler: Device wiped [ data rendered permanently irrecoverable through secure erasure ]\n", v, mode);
}
...
void lock_screen(){
    /* Use xdg-lock instead of Qubes-specific xflock4 */
    system("xdg-lock");
}
